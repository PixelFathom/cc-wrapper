...
export function DeploymentGuideModal({ taskId, trigger }: DeploymentGuideModalProps) {
  ...
  // Fetch deployment guide when modal opens
  useEffect(() => {
    if (open && taskId) {
...
- Restart services if needed`
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="w-[96vw] max-w-4xl max-h-[96vh] sm:max-h-[92vh] overflow-y-auto mx-auto p-4 sm:p-6">
        <DialogHeader className="space-y-3 pb-4 border-b border-border/30">
          <DialogTitle className="flex items-center gap-3 text-lg sm:text-xl font-semibold">
            <div className="bg-gradient-to-br from-orange-500/20 to-red-500/20 p-2.5 rounded-xl border border-orange-500/30">
          ...
          <p className="text-sm sm:text-base text-muted-foreground leading-relaxed">
            Instructions for deploying, restarting, and testing this project. This guide is shared with coding agents for context.
          </p>
        </DialogHeader>
        ...
        <div className="space-y-6 pt-4">
          ...
          {/* Footer Info */}
          <div className="bg-gradient-to-r from-blue-500/5 to-purple-500/5 rounded-lg p-4 border border-blue-500/20">
            <div className="flex items-start gap-3">
              <div className="bg-blue-500/20 p-2 rounded-lg shrink-0">
                <RocketIcon className="h-4 w-4 text-blue-400" />
              </div>
              <div className="space-y-2">
                <p className="text-sm font-semibold text-foreground">Agent Context</p>
                <p className="text-xs text-muted-foreground leading-relaxed">
                  This deployment guide is automatically included with every test case execution, providing coding agents with essential context about how to deploy, restart services, and verify functionality in your environment.
                </p>
              </div>
            </div>
          </div>
