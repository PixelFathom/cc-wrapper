...
test.describe('Git Init and Push Button', () => {
  test('should submit form when clicking git init && push button', async ({ page }) => {
    ...
    await page.fill('input#repo', 'git@github.com:username/cc-wrapper-2.git');
    ...
    expect(postData).toEqual({
      ...
      repo_url: 'git@github.com:username/cc-wrapper-2.git'
    });
  });

  test('should validate GitHub SSH repository URLs', async ({ page }) => {
    await page.goto('/');
    ...
    await expect(page.locator('text=Only GitHub SSH URLs are allowed')).toBeVisible();
    ...
    // Test valid URL
    await page.fill('input#repo', 'git@github.com:username/cc-wrapper.git');
    
    // Error should disappear and checkmark should appear
    await expect(page.locator('text=Only GitHub SSH URLs are allowed')).not.toBeVisible();
