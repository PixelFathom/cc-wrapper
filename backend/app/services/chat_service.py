        ...
        try:
            ...
            if task.deployment_port:
                deployment_context = (
                    ...
                    f"SERVICE URL: http://localhost:{task.deployment_port}\n\n"
                    f"⚠️ CRITICAL RULES:\n"
                    ...
                    f"   Run: curl -s -o /dev/null -w '%{{http_code}}' http://localhost:{task.deployment_port} || echo 'not running'\n"
                    ...
                    f"   Always confirm service responds on http://localhost:{task.deployment_port} before using Playwright MCP\n"
                    f"   Use: curl -I http://localhost:{task.deployment_port}\n\n"
                    f"STEP 4 - PLAYWRIGHT MCP TESTING:\n"
                    f"   Navigate to: http://localhost:{task.deployment_port}\n"
